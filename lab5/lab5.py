...
boost_1796.train(20, verbose=False)
#print 'house_1796_votes', house_1796_votes[0]
...
vote = [vote for vote in house_1796_votes if 'newspapers to be sufficiently dried before mailing' in vote['name'].lower()][0]
vote_id = int(vote['id'])
#print 'vote', vote, vote_id
federalists = [legislator for legislator in house_1796 if legislator['party'] == 'Federalist']
#print 'federalists', federalists
republicans = [legislator for legislator in house_1796 if legislator['party'] == 'Republican']
#print 'republicans', republicans

#print [boost_1796.classify(legislator) for legislator in house_1796]
#print [boost_1796.classify(republican) for republican in republicans]
#print [boost_1796.classify(federalist) for federalist in federalists]
#print federalists[20]
#print 'error rate', boost_1796.error_rate(house_1796, standardPartyClassifier)
...
#    print 'most_misclassified using classifier', classifier
#    print 'error rate', classifier.error_rate(house_1796, standardPartyClassifier)
    """
    ...
    """
    legislators = sorted(zip(classifier.data, classifier.data_weights), key=lambda tup: tup[1], reverse=True)[:n]
    return [legislator_info(x[0]) for x in legislators]
...
#classifiers_for_best_ensemble = ['svmr', 'maj', 'nb']
if __name__ == "__main__":
...
if __name__ == "__main__x":
    dataset = "breast-cancer"
    #for i in range(2, len(classifiers_for_best_ensemble) - 1):
    for c in combinations(classifiers_for_best_ensemble, 1):
        b_learners = {}
        for short_name in list(c):
            b_learners[short_name] = learners[short_name]
            #describe_and_classify(dataset, b_learners)
        print "Boosting with our suite of orange classifiers: %s" % ' '.join(c)
        print ("  accuracy: %.3f, brier: %.3f, auc: %.3f" % boosted_ensemble(dataset, b_learners, DATASET_STANDARDS[dataset]))
