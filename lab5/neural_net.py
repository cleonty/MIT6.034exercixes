...
class Neuron(DifferentiableElement):
    ...
    def compute_doutdx(self, elem):
        ...
        """
        my_weights = self.get_weight_nodes()
        my_inputs = self.get_inputs()
        for i in xrange(len(my_weights)):
            weight = my_weights[i]
            if weight == elem:
                x = my_inputs[i].output()
                y = self.output()
                return y * (1 - y) * x
            elif self.isa_descendant_weight_of(elem, weight):
                z = self.output()
                w = weight.get_value()
                doutx = self.my_inputs[i].dOutdX(elem)
                return z * (1 - z) * w * doutx
        return 0
...
class PerformanceElem(DifferentiableElement):
    ...
    def dOutdX(self, elem):
        ...
        o = self.my_input.output()
        d = self.my_desired_val
        doutx = self.my_input.dOutdX(elem)
        return (d - o) * doutx
...
def make_neural_net_two_layer():
    ...
    """
    seed_random()
    i0 = Input('i0', random_weight()) # this input is immutable
...
def make_neural_net_challenging():
    ...
    E = Neuron('F', [A,B,C,D,i0], [wAE,wBE,wCE,wDE,wE])
