...
class BoostClassifier(Classifier):
    ...
    def classify(self, obj):
        ...
        for (classifier, alpha) in self.classifiers:
            result += alpha * classifier.classify(obj)
        if result > 0:
            return 1
        else:
            return -1
    ...
    def orange_classify(self, obj):
        ...
        for (classifier, alpha) in self.classifiers:
            result += alpha * classifier.classify(obj)
        #print "result %f %f" % (result, sigmoid(20*result))
        result = sigmoid(20*result)
        return result
    ...
    def best_classifier(self):
        ...
        #print 'difference is ', set(self.base_classifiers) - set(self.classifiers)
    ...
    def step(self, verbose=False):
        ...
        * Update the weights based on the result of the classifier
        ...
        #print 'end step base_classifiers are', set(self.base_classifiers)
    ...
    def update_weights(self, best_error, best_classifier):
        ...
        for datum, dweight in zip(self.data, self.data_weights):
            if best_classifier.classify(datum) != self.standard.classify(datum):
                weight = 0.5 * 1.0/best_error * dweight
                sums[1] += weight
            else:
                weight = 0.5 * 1.0/(1.0 - best_error) * dweight
                sums[0] += weight
            weights.append(weight)
        #print 'do weights look good(1/2, 1/2)?', sums
        self.data_weights = weights
        #print "updated wights are", weights
