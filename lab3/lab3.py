...
## same depth.
def is_free_cell(board, cell):
    row = cell[0]
    col = cell[1]
    if row < 0 or col < 0 or row > 5 or col > 6 or board.get_cell(row, col) == board.get_other_player_id():
        return False
    else:
        return True

def num_open_ends(board, chain):
    dir = (chain[1][0] - chain[0][0], chain[1][1] - chain[0][1])
    rdir = (-dir[0], -dir[1])
    #print 'dirs', dir, rdir
    moves = ((chain[0][0]-dir[0], chain[0][1]-dir[1]),(chain[-1][0]-rdir[0], chain[-1][1]-rdir[1]))
    a = (is_free_cell(board, moves[0]), is_free_cell(board, moves[1]))
    count_possible_moves = a[0] + a[1]
    return count_possible_moves

def better_evaluate(board):
    if board.is_game_over():
        score = -1000
    else:
        f = lambda result, chain: result + 1 if (len(chain) == 3  and num_open_ends(board, chain)) else result
        score = board.longest_chain(board.get_current_player_id()) * 10
        my_3_open_chains_count = reduce(f, board.chain_cells(board.get_current_player_id()), 0)
        his_3_open_chains_count = reduce(f, board.chain_cells(board.get_other_player_id()), 0)
        score += (my_3_open_chains_count - his_3_open_chains_count) * 5
...
WHAT_I_FOUND_INTERESTING = "Evrything"
WHAT_I_FOUND_BORING = "Nothing"
NAME = "leonty"
EMAIL = "leonty@inbox.ru"
