...
def focused_evaluate(board):
    ...
    if board.is_win() == board.get_other_player_id():
        return -1001
    elif board.is_win() == board.get_current_player_id():
        return 1001
    return 5
...
def my_minimax(board, depth, eval_fn = basic_evaluate,
            get_next_moves_fn = get_all_next_moves,
            is_terminal_fn = is_terminal,
            verbose = True):
    """
    ...
    if verbose:
        print "MY MINIMAX: Decided on column %d with rating %d" % (best_val[1], best_val[0])
...
#    return my_minimax(board, depth, eval_fn, get_next_moves_fn, is_terminal_fn)
